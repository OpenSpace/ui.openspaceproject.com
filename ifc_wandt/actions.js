...
async function prepare() {
  ...
  try {
    ...
    // Turn off all trails
    await openspace.setPropertyValue(
    ...
    await openspace.setPropertyValueSingle(
      "Scene.Earth.Renderable.Layers.ColorLayers.ESRI_World_Imagery.Enabled",
      true,
    );
    await openspace.setPropertyValueSingle(
      "Scene.Earth.Renderable.Layers.ColorLayers.ESRI_World_Imagery.Fade",
      1.0,
    );

    // Turn off zoom limits
    await openspace.setPropertyValueSingle(
      "NavigationHandler.OrbitalNavigator.LimitZoom.EnabledMinimumAllowedDistance",
      false,
    );
...
async function setViewCupolaX(view = DEFAULT_VIEW_CUPOLA) {
  ...
  await openspace.setPropertyValueSingle("Scene.ISSCupola.Renderable.Fade", 1.0);
  ...
  let retargetValue = (
    await openspace.getPropertyValue(
...
async function hideBlackout() {
  ...
  let enabled = (
    await openspace.getPropertyValue("ScreenSpace.blackoutbrowser.Enabled")
  )[1];
  if (enabled) {
    if (fade > 0.5) {
      await openspace.setPropertyValueSingle(
...
async function isAssetLoaded(asset, show = true) {
  ...
  let filtered = Object.values(loadedAssets[1]).filter(
    (v) => v.split("\\").pop() == asset,
  );
    ...
    .then(async function (response) {
      if (!response.ok) {
        ...
        if ((await isAssetLoaded("blackoutbrowser.asset", false)) === false) {
          hideBlackoutDiv(
            "Add blackoutbrowser.asset and reload page if you want controls",
          );
        }
      }
    })
...
function hideBlackoutDiv(msg) {
  document.getElementById("blackoutDiv").innerHTML =
...
async function enableBlackoutControls() {
  ...
  let loadedAssets = await openspace.asset.allAssets();
  let filtered = Object.values(loadedAssets[1]).filter(
    (v) => v.split("\\").pop() == "blackoutbrowser.asset",
  );
  if (filtered.length == 0) {
  ...
  if (READ_KEY === undefined) {
    ...
    try {
      ...
      if (isNaN(numRes) === true) {
        // We got key back and not error code
...
async function setBlackoutValues() {
  ...
  const scale = (
    await openspace.getPropertyValue("ScreenSpace.blackoutbrowser.Scale")
  )[1];
  const position = (
    await openspace.getPropertyValue("ScreenSpace.blackoutbrowser.RadiusAzimuthElevation")
  )[1];
  ...
  try {
    ...
    if (couldUpdate === 1) {
      ...
      await openspace.setPropertyValueSingle(
        "ScreenSpace.blackoutbrowser.RadiusAzimuthElevation",
        position,
      );
